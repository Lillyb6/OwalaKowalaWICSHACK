...
const Navbar = () => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      if (currentUser) {
        ...
        if (userDoc.exists()) {
          ...
          const goal = 5; 
          setCurrentProgress(Math.round((done / goal) * 100));
  ...
  const sidebarStyle = {
    ...
    zIndex: 2000, padding: '80px 20px 20px 20px', display: 'flex', flexDirection: 'column', gap: '20px'
  };
      ...
      <div style={sidebarStyle}>
        ...
        <div className="px-2 mb-6">
          <div className="flex justify-between items-end">
            <span className="text-[#2d5a27] font-bold text-lg">Level 12</span>
            <span className="text-[10px] text-gray-500 font-bold uppercase tracking-wider">
              {Math.round((currentProgress / 100) * 5)} / 5 HP
            </span>
          </div>
